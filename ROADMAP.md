...
## v0.9: Simulation Refactor, Visualization & AWS Braket
...
Bloch sphere renderer, and broaden hardware support beyond IBM. A minor
release: additive (AWS Braket) and substantial internal refactors, all
backward-compatible.
...
## v0.12: Deprecation Removals & Breaking Cleanup

The pre-1.0 home for breaking changes. Deprecated aliases are removed here
once their deprecation window has elapsed, and future breaking cleanups
land here too, so v1.0 can ship a stable surface without itself breaking
anything. Each removal needs a CHANGELOG `### Removed` entry.
